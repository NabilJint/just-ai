...
var require_sha256 = __commonJS({
  "node_modules/.pnpm/fast-sha256@1.3.0/node_modules/fast-sha256/sha256.js"(exports, module) {
    ...
    (function(root, factory) {
      ...
      __name(hmac2, "hmac");
      exports2.hmac = hmac2;
      function fillBuffer(buffer, hmac3, info, counter2) {
        var num = counter2[0];
        if (num === 0) {
      ...
      function hkdf(key, salt, info, length) {
        ...
        for (var i = 0; i < length; i++) {
          if (bufpos === buffer.length) {
            fillBuffer(buffer, hmac_, info, counter2);
            bufpos = 0;
          }
          out[i] = buffer[bufpos++];
        }
...
// node_modules/.pnpm/@liveblocks+react-flow@3.19.2_@types+json-schema@7.0.15_@types+react-dom@19.2.3_@types+_9bc099c0eb6e7232c5cfe63dcf21e557/node_modules/@liveblocks/react-flow/dist/node.js
init_esm();

// node_modules/.pnpm/@liveblocks+core@3.19.2_@types+json-schema@7.0.15/node_modules/@liveblocks/core/dist/index.js
init_esm();
...
// node_modules/.pnpm/@liveblocks+react-flow@3.19.2_@types+json-schema@7.0.15_@types+react-dom@19.2.3_@types+_9bc099c0eb6e7232c5cfe63dcf21e557/node_modules/@liveblocks/react-flow/dist/lib/shared.js
init_esm();
...
// node_modules/.pnpm/@liveblocks+node@3.19.2_@types+json-schema@7.0.15/node_modules/@liveblocks/node/dist/index.js
init_esm();
...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../lib-DBDSP4ZG.mjs")).default;
}
...
var NODE_COLOR_PALETTE = [
  { fill: "var(--node-palette-neutral-fill)", text: "var(--node-palette-neutral-text)", stroke: "var(--node-palette-neutral-stroke)", minimap: "var(--node-palette-neutral-stroke)" },
  // Neutral dark
  { fill: "var(--node-palette-blue-fill)", text: "var(--node-palette-blue-text)", stroke: "var(--node-palette-blue-stroke)", minimap: "var(--node-palette-blue-stroke)" },
  // Blue
  { fill: "var(--node-palette-purple-fill)", text: "var(--node-palette-purple-text)", stroke: "var(--node-palette-purple-stroke)", minimap: "var(--node-palette-purple-stroke)" },
  // Purple
  { fill: "var(--node-palette-orange-fill)", text: "var(--node-palette-orange-text)", stroke: "var(--node-palette-orange-stroke)", minimap: "var(--node-palette-orange-stroke)" },
  // Orange
  { fill: "var(--node-palette-red-fill)", text: "var(--node-palette-red-text)", stroke: "var(--node-palette-red-stroke)", minimap: "var(--node-palette-red-stroke)" },
  // Red
  { fill: "var(--node-palette-pink-fill)", text: "var(--node-palette-pink-text)", stroke: "var(--node-palette-pink-stroke)", minimap: "var(--node-palette-pink-stroke)" },
  // Pink
  { fill: "var(--node-palette-green-fill)", text: "var(--node-palette-green-text)", stroke: "var(--node-palette-green-stroke)", minimap: "var(--node-palette-green-stroke)" },
  // Green
  { fill: "var(--node-palette-teal-fill)", text: "var(--node-palette-teal-text)", stroke: "var(--node-palette-teal-stroke)", minimap: "var(--node-palette-teal-stroke)" }
  // Teal
];
function isCanvasNodeShape(value) {
  return NODE_SHAPES.includes(value);
}
__name(isCanvasNodeShape, "isCanvasNodeShape");

// lib/node-id.ts
init_esm();
var counter = 0;
function generateNodeId(shape) {
  counter += 1;
  return `${shape}-${Date.now()}-${counter}`;
}
__name(generateNodeId, "generateNodeId");

// lib/design-agent-constants.ts
init_esm();
var AI_AGENT_USER_ID = "ghost-ai-agent";
var AI_AGENT_DISPLAY_NAME = "Ghost AI";
var AI_AGENT_CURSOR_COLOR = "#8b82ff";
var AI_AGENT_AVATAR_URL = "/favicon.png";
var CANVAS_LAYOUT_GAP_X = 120;
var CANVAS_LAYOUT_GAP_Y = 80;
var AI_PRESENCE_TTL_ACTIVE = 120;
var AI_PRESENCE_TTL_CLEAR = 3;
...
__name(defaultSizeForShape, "defaultSizeForShape");
function resolveNodeColor(shape, paletteIndex) {
  if (typeof paletteIndex === "number" && paletteIndex >= 0 && paletteIndex < NODE_COLOR_PALETTE.length) {
    return NODE_COLOR_PALETTE[paletteIndex];
  }
  return NODE_COLORS[shape] ?? DEFAULT_NODE_COLOR;
}
__name(resolveNodeColor, "resolveNodeColor");
...
function parseAction(raw) {
  ...
  switch (raw.type) {
    case "addNode": {
      ...
      return {
        type: "addNode",
        tempId: parseString(raw.tempId) ?? void 0,
        shape,
        label: parseString(raw.label) ?? void 0,
        x,
        y,
        width: parseNumber(raw.width) ?? void 0,
        height: parseNumber(raw.height) ?? void 0,
        paletteIndex: parseNumber(raw.paletteIndex) ?? void 0
      };
    }
    case "moveNode": {
    ...
    case "updateNodeData": {
      ...
      return {
        ...
        label: parseString(raw.label) ?? void 0,
        paletteIndex: parseNumber(raw.paletteIndex) ?? void 0,
        shape: shape && isCanvasNodeShape(shape) ? shape : void 0
      };
    }
    case "deleteNode": {
      const id = parseString(raw.id);
...
function parseDesignPlan(value) {
  if (!isRecord(value)) return null;
  const actionsRaw = Array.isArray(value.actions) ? value.actions : [];
  const actions = actionsRaw.map(parseAction).filter((action) => action !== null);
...
__name(extractJsonFromModelText, "extractJsonFromModelText");
function buildNodeFromAddAction(action) {
...
__name(buildNodeFromAddAction, "buildNodeFromAddAction");
function buildEdgeFromAddAction(action, resolveId) {
  const id = action.tempId && action.tempId.startsWith("e_") ? action.tempId : `e_${Math.random().toString(36).slice(2, 11)}`;
  return {
    id,
    type: "canvasEdge",
...
__name(buildEdgeFromAddAction, "buildEdgeFromAddAction");
function getDesignConstraintsPrompt() {
  return [
...
__name(getDesignConstraintsPrompt, "getDesignConstraintsPrompt");

// lib/canvas-flow.ts
var FLOW_STORAGE_KEY = "flow";
async function getCanvasSnapshotFromRoom(roomId) {
  const client = getLiveblocksClient();
  const doc = await client.getStorageDocument(roomId, "json");
...
__name(getCanvasSnapshotFromRoom, "getCanvasSnapshotFromRoom");
async function applyDesignActions(roomId, actions) {
  const client = getLiveblocksClient();
      ...
      for (const action of actions) {
        switch (action.type) {
          ...
          case "updateNodeData": {
            ...
            const color = shape && paletteIndex !== void 0 ? resolveNodeColor(shape, paletteIndex) : node.data?.color;
            flow.updateNodeData(id, {
...
// lib/design-agent-ai.ts
init_esm();
function summarizeCanvas(snapshot) {
  if (snapshot.nodes.length === 0 && snapshot.edges.length === 0) {
    return "The canvas is empty.";
  }
  const nodeLines = snapshot.nodes.map(
    (node) => `- ${node.id} (${node.type ?? "node"}): "${node.data?.label ?? ""}" at (${Math.round(node.position.x)}, ${Math.round(node.position.y)})`
  );
...
async function generateDesignPlan(prompt, snapshot) {
...
// lib/design-agent-liveblocks.ts
init_esm();
async function publishAiStatus(roomId, phase, message, runId) {
  const client = getLiveblocksClient();
  const event = {
    type: "ai-status",
    phase,
...
async function setAiPresence(roomId, options2) {
  const client = getLiveblocksClient();
  await client.setPresence(roomId, {
    userId: AI_AGENT_USER_ID,
    data: {
      cursor: options2.cursor ?? null,
      isThinking: options2.isThinking
    },
    userInfo: {
    ...
    ttl: options2.ttl ?? AI_PRESENCE_TTL_ACTIVE
  });
}
__name(setAiPresence, "setAiPresence");
async function clearAiPresence(roomId) {
...
__name(clearAiPresence, "clearAiPresence");
